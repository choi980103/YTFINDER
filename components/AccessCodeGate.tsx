...
function getInitialAuthState(): {
  ...
  if (typeof window === "undefined") {
    return { authorized: false, checking: true, hasStoredCode: false };
  }
  const storedCode = localStorage.getItem(STORAGE_KEY);
  if (storedCode) {
    return { authorized: false, checking: true, hasStoredCode: true };
  }
  if (!REQUIRE_ALL) {
    const hasApiKey = localStorage.getItem("yt_api_key");
    if (hasApiKey) {
      return { authorized: true, checking: false, hasStoredCode: false };
    }
  }
  return { authorized: false, checking: false, hasStoredCode: false };
}

export default function AccessCodeGate({ children }: AccessCodeGateProps) {
  const initial = getInitialAuthState();
  const [isAuthorized, setIsAuthorized] = useState(initial.authorized);
  const [isChecking, setIsChecking] = useState(initial.checking);
  ...
  useEffect(() => {
    if (!initial.hasStoredCode) return;
    const storedCode = localStorage.getItem(STORAGE_KEY);
  ...
  return (
    ...
    <div className="mx-auto max-w-4xl">
      ...
      <section className="relative overflow-hidden px-4 pb-16 pt-20 text-center sm:px-6 sm:pt-28 sm:pb-20">
        ...
        {/* 배지 */}
        <div className="relative mb-8 inline-flex items-center gap-1.5 rounded-full border border-[#00e5a0]/20 bg-[#00e5a0]/10 px-4 py-1.5 text-xs font-bold text-[#00e5a0] sm:text-sm">
      ...
      <section className="px-4 py-14 sm:px-6 sm:py-20">
        ...
        <div className="flex flex-col gap-4">
          {[
            ...
            { icon: "⚡", bg: "rgba(251,191,36,0.1)", t: "오늘의 Top 300 영상", d: "매일 업데이트되는 인기 쇼츠 영상 순위. 지금 가장 핫한 콘텐츠가 무엇인지 실시간으로 확인하세요.", tag: "인기 기능", tagColor: "#fbbf24" },
      ...
      <section className="px-4 py-14 text-center sm:px-6 sm:py-20">
        ...
        <div className="mx-auto grid max-w-2xl gap-3 text-left sm:grid-cols-2">
          {[
            ...
            { icon: "🔥", t: "Top 300", d: "매일 갱신되는 인기 쇼츠 영상 순위. 최근 3일 이내 핫한 영상만 선별" },
            ...
            return (
              <div
                ...
                className={`relative flex flex-col rounded-2xl border p-6 transition-colors ${
                  highlighted
                    ? "border-[#00e5a0]/40 bg-[#00e5a0]/[0.04]"
                    : "border-white/[0.06] bg-white/[0.02] hover:bg-white/[0.04]"
                }`}
              >
                {plan.badge && (
                  <span
                    className={`absolute -top-2.5 left-5 rounded-full px-2.5 py-0.5 text-[10px] font-bold tracking-wider ${
                      highlighted
                ...
                <div className="mt-3 flex items-baseline gap-1.5">
                  <span className="text-2xl font-black text-white sm:text-3xl">{formatKRW(plan.price)}</span>
                  {plan.discount > 0 && (
                ...
                <ul className="mt-5 flex-1 space-y-2 text-xs text-zinc-300">
                  ...
                  <li className="flex items-start gap-1.5">
                    <span className="text-[#00e5a0]">✓</span> 언제든 해지 · 자동 갱신 OFF 가능
                  </li>
                  <li className="flex items-start gap-1.5">
                    <span className="text-[#00e5a0]">✓</span> 향후 기능 업데이트 무료
                  </li>
                </ul>
                ...
                <Link
                  ...
                  className={`mt-6 block rounded-xl px-4 py-2.5 text-center text-sm font-semibold transition-all ${
                    highlighted
                      ? "bg-gradient-to-r from-[#00e5a0] to-[#06b6d4] text-[#0a0a0f] hover:opacity-90"
          ...
          <table className="w-full min-w-[600px] border-collapse text-sm">
            ...
            <tbody className="text-zinc-300">
              {[
                ...
                { label: "한국 쇼츠 특화", a: false, b: "일부", c: true, d: false, yt: true },
              ].map((row) => (
              ...
              <tr className="border-t border-white/[0.04]">
                ...
                <td className="px-3 py-3 text-center bg-[#00e5a0]/[0.03] text-sm font-bold text-[#00e5a0]">월 29,900원~</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <section className="px-4 py-14 sm:px-6 sm:py-20">
        ...
        <h2 className="mb-8 text-2xl font-black sm:text-3xl">구독에 <span className="bg-gradient-to-r from-[#00e5a0] to-[#06b6d4] bg-clip-text text-transparent">포함된 것들</span></h2>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {["꿀통 지수 분석", "글로벌 트렌드", "Top 300 영상", "채널 비교 분석", "벤치마크 기능", "채널 상세 분석", "즐겨찾기 & 메모", "향후 업데이트 무료"].map((item) => (
      ...
      <section className="px-4 py-14 sm:px-6 sm:py-20">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {[
            ...
            { text: "vidIQ 쓰다가 갈아탔는데, 한국 쇼츠에 특화되어 있어서 훨씬 실용적이에요. 가격도 합리적이라 부담 없습니다.", author: "구독자 10만 유튜버 B님" },
      ...
      <section className="px-4 py-14 sm:px-6 sm:py-20">
        ...
        <div className="flex flex-col gap-3">
          {[
            { q: "구독은 어떻게 갱신되나요?", a: "선택하신 기간(1/3/6/12개월)이 끝나기 전 자동으로 갱신됩니다. 결제 페이지 또는 이메일 한 번이면 언제든 자동결제 해지 가능하며, 이미 결제된 기간은 만료일까지 정상 이용 가능합니다." },
            { q: "환불이 가능한가요?", a: "결제 후 7일 이내 미사용이면 전액 환불, 7일 이내 사용 시작했다면 일할 차감 환불해드립니다. 7일 이후엔 환불이 제한될 수 있습니다. 자세한 내용은 환불정책을 참고해주세요." },
            { q: "별도 프로그램 설치가 필요한가요?", a: "아니요! 100% 웹 기반 서비스라 브라우저만 있으면 PC, 태블릿, 모바일 어디서든 사용 가능합니다." },
            { q: "YouTube API 키는 뭔가요? 어렵지 않나요?", a: "Google에서 무료로 발급받을 수 있는 키입니다. 구독 시작 후 상세한 발급 가이드를 제공해드리며, 5분이면 완료됩니다." },
