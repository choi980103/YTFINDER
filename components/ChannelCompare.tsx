        ...
        <div className="mt-3">
          ...
          <div className="mb-4 rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
            <div className="mb-2 flex items-center justify-between">
              <span className="text-xs text-zinc-400">비교할 채널 선택 (최대 3개)</span>
              {selected.length > 0 && (
                <button onClick={() => setSelected([])} className="text-[11px] text-zinc-500 hover:text-zinc-300">
                  초기화
                </button>
              )}
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/[0.06]">
                    <th className="px-4 py-3 text-left text-[10px] font-medium uppercase tracking-widest text-zinc-500">항목</th>
                ...
                <tbody>
                  ...
                  <tr className="border-b border-white/[0.04]">
                    <td className="px-4 py-2.5 text-xs text-zinc-400">떡상 지수</td>
                    {selectedChannels.map((ch) => {
                  ...
                  {/* 예상 영상당 수익 */}
                  <tr>
                    <td className="px-4 py-2.5 text-xs text-zinc-400">예상 영상당 수익</td>
                    {selectedChannels.map((ch) => {
          ...
          {selectedChannels.length < 2 && selectedChannels.length > 0 && (
            <p className="text-xs text-zinc-500">채널을 1개 더 선택하면 비교할 수 있어요</p>
          )}
        </div>
      )}
    </div>
